...
impl FormatImpl for INT8Format {
    ...
    fn dequantize(
        ...
        // For INT8, we need 1 byte per element
        ...
        // 2MB is a reasonable threshold for INT8 data
        ...
        parallel_process(
            ...
            num_blocks,
            block_size,
            block_size, // For INT8, bytes_per_block equals elements_per_block
            parallel_threshold,
...
/// Function to dequantize a range of INT8 values with optimized memory access
#[inline(always)]
fn dequantize_int8_blocks(
    data: &[u8],
    base_offset: usize,
    start_block: usize,
    end_block: usize,
    ...
    // For INT8, element count equals byte count
    ...
    // For INT8 format, we just need to convert each byte to i8 and then to f32
    for i in 0..element_count {
        // Interpret as signed int8
        let int8_val = data[byte_offset + i] as i8;
        result[start_element + i] = int8_val as f32;
    }
}
